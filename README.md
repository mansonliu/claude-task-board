...
- 📌 **任務若有產出可線上看的網址（GitHub Pages、Blogger 文章、公開 HTML 等），把網址放進 `note`**，使用者要能從看板直接點開成品。`index.html` 的 `noteHtml()` 會自動把 http(s) 網址轉成可點連結（停在 `｜，。、）」` 等字元），所以網址後面用 `｜` 接其他說明即可。
- 每次 `update` 卡片時**保留已有的網址**，別覆蓋掉。
